...
std::string BuildIndexFile(std::vector<std::string> const & filesForIndex)
{
  ...
  std::string content =
      ...
      "<Document>\n"
      "<name>Streifzug Bookmarks and Tracks</name>\n";
...
{
  auto const kmzFileName = "Streifzug_backup_" + std::to_string(base::GenerateYYMMDD(time(nullptr)));
  ...
  for (auto const & kmlToExport : *collection)
  {
    ...
    if (!strings::IsASCIIString(fileName))
      fileName = "Streifzug_" + std::to_string(suffix++);
