...
**Status:** In progress (SP-054–SP-060 Accepted 2026-08-15–18; SPD-040–045
locked; SP-061 Phase 6 end-to-end validation **In progress**)
**Depends on:** Phase 3
...
| Options | `routing::StreetExplorationRoutingOptions` | `m_mode` Neither/Prefer/Avoid + `m_strength` 0–100 (default 50). Settings key `street_exploration_routing_mode` is source of truth; legacy `street_exploration_routing_enabled` dual-writes `"true"` iff Prefer. Avoid stored applies exclusion (SP-057); dual-write enabled false for Avoid |
...
| Walk/bike options UI | `WalkingOptionsFragment`, `CyclingOptionsFragment` | Prefer + Avoid (warning before Avoid) + seekbar via `include_street_exploration_prefer.xml`. Avoid visible on walk/bike only |
...
tabs now expose Prefer + Avoid (warning before Avoid) + seekbar (SP-056 /
SP-058); Avoid chrome is on walk/bike and is no longer hidden until SP-058.
The audit’s “driving-options surface only” snapshot is stale. The single-MWM
overlay remains renderer-only; weight lookup consults the segment MWM `.pix`
when installed (SPD-045).

**Difference from the product spec:** Prefer and Avoid are on walking and
cycling surfaces (§17.2, §17.3, §34, SPD-041). Strength slider is not in
the spec; **SPD-041** keeps it for V1. **SPD-042** records two V1
divergences: Avoid excludes only fully explored edges (`exploredRatio == 1`),
and no-route fallback is Prefer+strength rather than the §17.3 / §31
min-connection pair.
