...
**Status:** Planned
**Depends on:** SP-049 Accepted (layout D8); SP-044 / SP-045 tools Accepted
...
| `--data-version` | Must match countries `"v"` (e.g. `260714`) |
...
1. Load countries JSON; assert `"v"` == `--data-version` and `"map_series"` ==
   `--map-series` (fail closed on mismatch — do not rewrite version casually).
2. Run `inject_spa_meta` logic on a **copy**: advertise iff `{spa-dir}/{id}.spa`
   exists; omit both keys otherwise; strip stale keys.
3. For each advertised leaf: verify spa file size matches injected `"spa"`;
   recompute SHA-1 and assert equals `"spa_sha1_base64"`.
4. If `--include-mwm`: for each leaf that will be served (advertised spa leaves
   and/or an optional MWM allowlist), require `{mwm-dir}/{id}.mwm` exists; copy
   or hardlink into version dir; verify size/hash against countries `"s"` /
   `"sha1_base64"` (**fail closed** on mismatch — wrong MWM version is worse
   than missing spa).
5. Write `meta/maps.json` with at least:

   ```json
   {
     "map-series": {
       "2026.06.28": { "latest": 260714, "status": "current" }
     }
   }
   ```

   (Match the field names `ParseServerMapsAndGetLatestVersion` actually reads:
   `"map-series"` / `"latest"` / `"status"`.)
6. Optionally `sign_file(countries.txt, secret_key)`.
7. Emit an inventory JSON next to the tree (not served by default): leaf id,
   mwm bytes, spa bytes, advertised yes/no — for debug and SP-053 evidence.
...
2. Injected spa size/hash match on-disk `.spa`; MWM size/hash match when
   included.
3. Version / series mismatch fails closed.
4. `--spa-only` mode produces countries + spa without requiring MWM copies.
5. With `--secret-key`, `.sig` verifies with the corresponding public key.
6. `--verify-only` / `--dry-run` work; inventory written.
7. Python unit tests cover inject+layout happy path and hash mismatch failure.
8. Docs recipe recorded; agent does not mark Accepted.
...
| Series / version mismatch vs countries | Fail |
