...
| HTTP failure keeps pending and advances `next_allowed` by 900s+jitter so `MaybeUpload` cannot retry-storm in the same window. Covered by `CompetitionUpload_HttpFailureDoesNotRetryStorm`. | Keep. Cadence gate is the retry control. |
| Dead `ExploreStatsService::BuildUploadJson` still calls `IdentityStore::GetOrCreateDeviceId()`. `TryUpload` does not call it. | Leave unused. Do not restore `/stats/upload` or this payload. Delete in a later cleanup if product wants the symbol gone. |
| `QueryCompetitionOwnership` uses `base::SecondsSinceEpoch()` for recency weights, not the snapshot `nowUnix` used for weekly city counts. Production clocks match; fake-clock scores via the manager snapshot still follow wall time. | Leave unless SP-072 grows an injected now into the ownership query. Do not upload extra fields to compensate. |
