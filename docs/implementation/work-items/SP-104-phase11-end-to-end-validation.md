...
operator CLI, identity, origin, Finland artifacts, and no Streifzug map
...
- Traffic: hosts-file/DNS block of Streifzug map peers **or** log-based
...
- Any Streifzug map fetch on the recorded stock path → Fail P1.
...
| P1 stock path | No Streifzug map fetch recorded on template / `configure.sh` / `map_pipeline`. Debug `prepare_spa_debug_root` is not the stock path |
