...
/*! Performs an endianness conversion on \a val.
    ...
    Named planck_byteswap to avoid C++23 std::byteswap ambiguity. */
template<typename T> inline void planck_byteswap (T &val)
...
class bifstream: public std::ifstream
  ...
  public:
    ...
    template<typename T> bifstream &operator>> (T &data)
      ...
      if (doswap) planck_byteswap (data);
      return *this;
      }
